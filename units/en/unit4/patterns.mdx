...
Subagents aren't random, they follow predictable patterns. Understanding these patterns helps you design multi-agent workflows efficiently.
...
The four patterns — fan-out/fan-in for parallel independent tasks, pipeline for sequential stages, supervisor for coordinating multiple specialists, and swarm for collaborative multi-perspective design — cover most multi-agent workflows. Use subagents when you have 10+ files to process or 3+ independent pieces of work. Stick with a single agent for small tasks and high-dependency workflows.

Next, let's see how to actually use subagents in Claude Code and Codex.
