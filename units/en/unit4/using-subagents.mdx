...
The approach differs between Claude Code and Codex, but the core idea is the same: a parent agent delegates a scoped task to a child, often in parallel.
...
In Claude Code, use conversational invocation, custom agents in `.claude/agents/`, CLAUDE.md policies, and background execution with Ctrl+B. In Codex, request subagents in natural language with three built-in types (default, worker, explorer), define custom TOML agents, monitor with `/agent`, and use CSV batch jobs for parallel work. Both platforms benefit from subagents when you have many files, several independent tasks, or need a fresh perspective. For parallel file edits, isolate work with git worktrees.
