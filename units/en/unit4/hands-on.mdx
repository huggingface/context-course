...
A researcher agent explores and documents, an implementer writes code, and security and performance reviewers sign off in parallel. Define the specialists in `.claude/agents/*.md` or `.codex/agents/*.toml`, and keep the workflow in `CLAUDE.md` or `AGENTS.md`. Hooks and runtime settings are for automation around the workflow, not for driving subagent orchestration.
