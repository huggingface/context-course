...
Gradio makes it incredibly easy to create MCP servers that also have a beautiful web interface. With one line of code, you get both a user-friendly UI and an MCP server that agents can use.
...
### Authentication for Secured MCP Servers
...
with gr.Blocks(auth=authenticate) as demo:
    gr.Markdown("# Protected MCP Server")
    
    def secret_operation() -> str:
        """Secret operation (authenticated access only)."""
        return "This is secret data"
    
    gr.Button("Get Secret").click(secret_operation, outputs="text")

if __name__ == "__main__":
    demo.launch(mcp_server=True)
```

Only authenticated clients can access the MCP endpoint.
...
Adding `mcp_server=True` to your Gradio launch gives you both a web UI and an MCP server automatically. Type hints and docstrings are essential — Gradio uses them to generate MCP tool schemas. Deploy to Hugging Face Spaces for free, public MCP servers where each function becomes a tool agents can call. Use `@gr.api()` for MCP-only functions and `@gr.mcp.resource()` for read-only data. Keep tools fast and handle errors gracefully to ensure reliable agent interactions.

Next, let's build a complete hands-on project from start to finish.
