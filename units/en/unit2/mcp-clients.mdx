...
Code agents are MCP clients that discover and use servers automatically. Your job is configuring which servers each agent can access. Let's see how to do that for Claude Code, Codex, and OpenCode.

## How Code Agents Act as MCP Clients

When you configure an MCP server for an agent, the agent gains access to that server's tools and resources immediately. The agent's built-in MCP client discovers what tools and resources the server provides, integrates them into the agent's available capabilities, routes requests to the appropriate server when needed, and handles errors gracefully. From the agent's perspective, tools from MCP servers are just as available as built-in functions.
...
Code agents act as MCP clients that discover and use servers automatically. Claude Code uses `claude mcp add`, Codex uses `codex mcp add` or `config.toml`, and OpenCode uses `opencode.json` — but all follow the same pattern of naming a server, specifying a transport, and providing a command or URL. Use stdio transport for local servers and Streamable HTTP for remote ones. Environment variables protect secrets. When debugging, list servers and test connections systematically.

Next, let's see how Gradio makes it easy to build MCP servers with a web UI.
