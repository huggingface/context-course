...
In Unit 2, you built a text-processor MCP server and deployed it to Hugging Face Spaces. Now let's turn that work into a **plugin**. For Claude Code and Codex, that means packaging the server reference with skills and plugin metadata. For OpenCode, it means writing a native plugin module and keeping MCP configuration beside it.

This is how plugins work in practice: manifest-first platforms wrap existing tools with skills and config, while code-first platforms expose behavior through plugin code.

![Plugin development lifecycle from creation through distribution and use](https://huggingface.co/datasets/mcp-course/images/resolve/main/unit3/plugin-lifecycle.svg)
...
Notice what's different from Unit 2: no server code here. For Claude Code and Codex, the plugin **references** the MCP server you already built and deployed. OpenCode's native plugin branch below adds behavior in code and can still point at the same server separately if you want both surfaces.

## Step 1: Write the Skills

This step applies directly to **Claude Code** and **Codex**, where plugins bundle `skills/`. If you're following the OpenCode branch, skip ahead to Step 2: OpenCode plugins are code modules, not skill bundles.
...
Claude Code and Codex use manifests to declare what the plugin provides. OpenCode uses a plugin module instead. The entry point differs by platform.
...
```
What are the keywords in the README of this project?
```
...
Claude Code and Codex use manifest-first plugins: `plugin.json` points to bundled `skills/` and optional `.mcp.json` or `.app.json` files. OpenCode plugins are different: they are JS/TS modules loaded from `.opencode/plugins/` or npm, and any skills or MCP servers live beside that system instead of inside it. You built the server in Unit 2; here you turned that work into reusable plugin behavior using the install surface each platform actually documents.

Next, test your knowledge with a quiz, then learn how to install and use plugins across agents.
