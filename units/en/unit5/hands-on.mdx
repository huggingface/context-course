...
def _normalize(body: dict[str, Any], headers: dict[str, str]) -> dict[str, Any]:
    """Map Claude Code / Codex / OpenCode payloads to one shape."""
    platform = (
        body.get("platform")
...
async def event(req: Request):
    ...
    # Return an empty directive so Claude Code / Codex hooks don't block anything.
    return JSONResponse({})
...
with gr.Blocks(title="Agent Activity Dashboard") as ui:
    ...
    gr.Markdown(
        "Point your Claude Code, Codex, or OpenCode hooks at "
