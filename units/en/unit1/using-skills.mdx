...
Now that you understand what skills are, let's learn how to actually use them with your code agent. The Agent Skills Specification ensures the SKILL.md format works across agents — the main difference is where each agent looks for skills and how you install them.
...
Codex scans these locations in priority order (repository first, then user, then admin).
...
Skills are distributed as plugins via marketplaces. Add a marketplace, then install the plugin containing the skill:

```bash
# Add the Hugging Face skills marketplace
claude plugin marketplace add huggingface/skills

# Install the hf-cli plugin
claude plugin install hf-cli@huggingface-skills

# Confirm it is installed
claude plugin list
```

Plugin skills are namespaced (e.g., `/huggingface-skills:hf-cli`) to prevent conflicts. Browse available plugins in the marketplace UI or with the `claude plugin` CLI.
...
Or check installed plugins with:
...
You now know how to install and use skills across Claude Code, Codex, and OpenCode. Ready to build your first skill? Let's move to the next lesson.
