#!/usr/bin/env python3
"""Print a Codex kickoff prompt for inference optimization."""
...
Use the checked-in Codex agents from .codex/config.toml.
...
Spawn `benchmarker` to measure a reproducible baseline. Then spawn
`optimizer` for up to {max_candidates} one-variable speed candidates. For the
best candidate, spawn `benchmarker` to run the same benchmark and append the
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Print a Codex kickoff prompt for inference optimization."
    )
    parser.add_argument(
        "--target",
